...
public class CartDeleteSelected extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(cartnoArr == null || cartnoArr.length == 0) {
        	request.setAttribute("message", "삭제할 상품을 선택하세요");
        	request.setAttribute("loc", request.getContextPath()+"/order/cart.lp");
        	setRedirect(false);
        	setViewPage("/WEB-INF/msg.jsp");
        	return;
        }
        int deletedCount = cdao.deleteSelected(userid, cartnoArr);
		
        request.setAttribute("message", "선택 상품 " + deletedCount + "개를 삭제했습니다.");
        request.setAttribute("loc", request.getContextPath() + "/order/cart.lp");
        setRedirect(false);
        setViewPage("/WEB-INF/msg.jsp");
	}
