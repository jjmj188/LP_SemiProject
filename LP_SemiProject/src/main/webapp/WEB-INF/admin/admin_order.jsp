  ...
  <div class="admin-container">
    ...
    <section class="admin-content">
      ...
      <div class="filter-container">
          <button type="button" 
                  class="filter-btn ${empty requestScope.status ? 'active' : ''}" 
                  onclick="location.href='<%= ctxPath %>/admin/admin_order.lp'">
              전체
          </button>
          
          <button type="button" 
                  class="filter-btn ${requestScope.status == '배송준비중' ? 'active' : ''}" 
                  onclick="location.href='<%= ctxPath %>/admin/admin_order.lp?status=배송준비중'">
              배송준비중
          </button>
          
          <button type="button" 
                  class="filter-btn ${requestScope.status == '배송중' ? 'active' : ''}" 
                  onclick="location.href='<%= ctxPath %>/admin/admin_order.lp?status=배송중'">
              배송중
          </button>
          
          <button type="button" 
                  class="filter-btn ${requestScope.status == '배송완료' ? 'active' : ''}" 
                  onclick="location.href='<%= ctxPath %>/admin/admin_order.lp?status=배송완료'">
              배송완료
          </button>
      </div>
      ...
      <table class="order-table">
        <colgroup>
            <col width="8%">  <col width="10%"> <col width="*">   <col width="15%"> <col width="10%"> <col width="8%">  <col width="22%"> </colgroup>
        <thead>
            <tr>
                <th>주문번호</th>
                <th>받는 분</th>
        ...
        <tbody>
            <c:if test="${empty requestScope.orderList}">
                <tr>
                    <td colspan="7" style="padding:50px; color:#777;">주문 내역이 없습니다.</td>
                </tr>
            </c:if>
